...
import me.andyreckt.uhc.utilties.spectator.SpectatorData;
...
public class SpectatorManager extends Manager {
    ...
    public void setSpectators(Map<UUID, SpectatorData> spectators) {
        this.spectators = spectators;
    }

    public List<UUID> getDeathInventory() {
        return deathInventory;
    }

    public void setDeathInventory(List<UUID> deathInventory) {
    ...
    public void handleMove(Player player, Location from, Location to) {
        ...
        if(to.getBlockY() <= 40) {
            ...
            player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't xray for other players :P."));
        }
        /*
        if(player.hasPermission(Permission.XENON_PERMISSION)
            ...
            if(!UHCUtils.maxSpecSize(to, 500) && UHCUtils.maxSpecSize(from, 500)) {
                player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't spectate more than &l500&c blocks."));
            ...
            if(!UHCUtils.maxSpecSize(to, 400) && UHCUtils.maxSpecSize(from, 400)) {
                player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't spectate more than &l400&c blocks."));
                player.teleport(from);
            }
        } else if(player.hasPermission(Permission.TITANIUM_PERMISSION)) {
            if(!UHCUtils.maxSpecSize(to, 300) && UHCUtils.maxSpecSize(from, 300)) {
                player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't spectate more than &l300&c blocks."));
                player.teleport(from);
            }
        } else if(player.hasPermission(Permission.NITROGEN_PERMISSION)) {
            if(!UHCUtils.maxSpecSize(to, 200) && UHCUtils.maxSpecSize(from, 200)) {
                player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't spectate more than &l200&c blocks."));
                player.teleport(from);
            }
        } else if(player.hasPermission(Permission.HYDROGEN_PERMISSION)) {
            if(!UHCUtils.maxSpecSize(to, 100) && UHCUtils.maxSpecSize(from, 100)) {
                player.sendMessage(me.andyreckt.uhc.utilties.Color.translate("&cYou can't spectate more than &l100&c blocks."));
                player.teleport(from);
            }
        }*/ else {
            if(!UHCUtils.maxSpecSize(to, 100) && UHCUtils.maxSpecSize(from, 100)) {
    ...
    public void handleRandomTeleport(Player player) {
        ...
        if(online.size() != 0) {
            ...
            player.sendMessage(Color.translate("&eYou have been randomly teleported to &9" + target.getName() + "&e."));
        }
    }
    ...
    public Inventory getInventory(int page) {
        ...
        toLoop.forEach(data -> {
            if(toLoop.indexOf(data) >= page * 9 - 9 && toLoop.indexOf(data) < page * 9) {
                ...
                builder.name("&7" + name);
                builder.lore("&eClick to teleport to the &9" + data.getRealName());
